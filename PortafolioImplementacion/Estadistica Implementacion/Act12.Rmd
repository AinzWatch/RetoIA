---
title: "12. Regresión Lineal - Análisis de los errores"
...
curve(dnorm(x,mean=mean(Modelo2$residuals),sd=sd(Modelo2$residuals)), from=-
...
abline(h=0, col="red")
```
El Grafico muestra homocedasticidad y simetria


## 2. No te olvides de incluir las hipótesis en la pruebas de hipótesis que realices.

En el caso donde nuestra $\alpha$ tiene un valor de 0.05 las hipotesis quedarian

Homocedasticidad:
El valor de P es menor a Alpha asi que la Hipotesis incial  se rechaza y ademas como la varianza de errores no es constante se mantiene la Hipotesis alternativa/H1.

Independencia:
El valor de P es menor a Alpha asi que la Hipotesis inicial no se rechaza, así que se identifica que los valores no estan autocorrelacionados
  

## 3. Interpreta en el contexto del problema cada uno de los análisis que hiciste.

En los analissi que realice se demuestra que los datos tienen independencia, asi que podemos identificar que la independencia podria estar por el valor de sexo, donde dependiendo del sexo los Pesos de Hombres y Mujeres no se autocorrelacionan.
...
Las grafica de QQPlot quedan iguales, mientras que las graficas de residuos tienen la linea roja un poco más distorisionada en esta ultima grafica
...
Al final como ya habiamos identificado el mejor modelo para regresión lineal es el Modelo 2, ya que podemos ver por las graficas de residuos muestra homocedasticidad y simetria, ademas que la grafica de QQPlot muestra que los datos estan Normalizados.
